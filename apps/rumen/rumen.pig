...
--
A     = load '$reads' using gov.jgi.meta.pig.storage.FastaBlockStorage as (offset: int, b:bag {s:(id: chararray, d: int, seq: chararray)});
...
-- D: {group: chararray,C: {blastmatches::geneid: chararray,id::id: chararray},READS: {b::id: chararray,b::d: int,b::seq: chararray}}

E     = foreach D generate FLATTEN(C.blastmatches::geneid), FLATTEN(reads); describe E;
-- E: {blastmatches::geneid::blastmatches::geneid: chararray,READS::b::id: chararray,READS::b::d: int,READS::b::seq: chararray}

F     = group E by $0 PARALLEL 100; describe F;
-- F: {group: chararray,E: {blastmatches::geneid::blastmatches::geneid: chararray,READS::b::id: chararray,READS::b::d: int,READS::b::seq: chararray}}
...
H     = foreach F generate group, gov.jgi.meta.pig.aggregate.VELVET($1, 1, $0);
dump H;

--contigs     = foreach F generate group, gov.jgi.meta.pig.aggregate.VELVET($1, 1, $0) as {geneid: chararray, contigs: {tuple: (contig: chararray)}};
describe H;

contigs = foreach H generate group, FLATTEN($1);
describe contigs;
--contigs: {group: chararray,contigs: {tuple: (seq: chararray)}}
