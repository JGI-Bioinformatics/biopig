...
DEFINE KMERS gov.jgi.meta.pig.generator.KmerGenerator();

-- load contig file
A = load '$contigs' using gov.jgi.meta.pig.storage.FastaStorage as (contigid: chararray, d: int, seq: chararray);

-- generate kmer index of the contig ends
B = foreach A generate FLATTEN(KMERS(seq)) as (kmer:chararray), contigid;

-- load the reads
R = load '$reads' using gov.jgi.meta.pig.storage.FastaStorage as (readid: chararray, d: int, seq: chararray);
